    ...
    useEffect(() => {
        registerHandlers({
            csv: () => {
                ...
                exportToCSV(cols, d.data, `gender-metrics-${yr || 'all'}`);
            },
            png: async () => {
                if (!pageRef.current) return;
                await exportToPNG(pageRef.current, `gender-report-${stateRef.current.selectedYear || 'all'}.png`);
            },
            pdf: async () => {
                ...
                await exportToPDF({
                    title: `${tr('report_title')} — ${tr('gender')}`,
                    subtitle: tr('gender_page_subtitle'),
                    year: yr, faculty: fac, generatedAt: new Date().toLocaleString(),
                    kpis: [
                        { label: tr('kpi_avg_female_pct'), value: `${d.summary.avg_female_pct}%` },
                        { label: tr('kpi_avg_male_pct'), value: `${d.summary.avg_male_pct}%` },
                        { label: tr('kpi_women_leadership'), value: d.summary.avg_women_leadership_pct != null ? `${d.summary.avg_women_leadership_pct}%` : 'N/A' },
                    ],
                    sections, t: tr,
                });
            },
